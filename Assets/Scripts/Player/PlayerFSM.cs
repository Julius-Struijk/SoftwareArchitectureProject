...
{
    /// <summary>
    /// Top-level player FSM. Wires the four concrete states together. Transition
    /// order is priority-sensitive (NextState returns the first matching transition):
    /// 1. Dead beats everything (HP == 0)
    /// 2. Hurt beats normal flow (a hit was logged this frame)
    /// 3. Move ↔ Idle natural flow
    /// 4. Hurt → Idle when stun ends
    ///
    /// Attack is not a state. PlayerController publishes attack events per-frame
    /// using AttackCooldown (so the player can move and attack at the same time).
    /// </summary>
